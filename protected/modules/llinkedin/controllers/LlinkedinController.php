...
class LlinkedinController extends Controller {
    ...
    public function actionIndex() {
		...
		$config['callback_url'] = trim($ll->url);
        ...
        if (isset($_GET['code'])) {
            ...
            $return = json_decode($this->post_curl($url, $param, "POST"), true);
			if (isset($return['error'])) {
                echo 'Some error occured<br><br>' . $return['error'] . '<br><br>Please Try again.';
            } else {
				...
				if(isset($User->error)) {
					die('Some error occured<br><br>' . $User->error . '<br><br>Please Try again.');
				}
    ...
    public function post_curl($url, $param = "", $method="GET") {
		$curl = curl_init();
		...
		if ($err) {
			...
			if(isset($response['error'])) {
				return json_encode(array('error' => $response['error_description']));
			}
			return $response;
		}
    }
